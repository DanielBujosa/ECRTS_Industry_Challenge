...
for i = 1:length(message_struct)
    ...
    %if type == "ClassA"
    %    if (message_struct(i).priority < 4)
    %        if (message_struct(i).deadline <= 4000)
    %            fprintf(file, 'priority = Class%s\n','A');
    %        elseif (message_struct(i).deadline <= 8000)
    %            fprintf(file, 'priority = Class%s\n','B');
    %        else
    %            fprintf(file, 'priority = Class%s\n','C');
    %        end
    ...
    %    end
    %if type == "ClassA"
    %    fprintf(file, 'priority = Class%s\n',prios(message_struct(i).priority));
    %if type == "ClassA"
    %    if (message_struct(i).priority < 4)
    %        fprintf(file, 'priority = Class%s\n',prios(message_struct(i).priority));
    %    else
    ...
    if type == "ClassA" && message_struct(i).deadline <= 8000
        fprintf(file, 'priority = Class%s\n','A');
    elseif type == "ClassA" && message_struct(i).deadline <= 16000
        fprintf(file, 'priority = Class%s\n','B');
    elseif type == "ClassA"
        fprintf(file, 'priority = Class%s\n','C');
    else
